...
class AsyncSocket {
    ...
    suspend fun connect(remote: SocketAddress) {
        suspendCancellableCoroutine { cont ->
            socket.connect(remote, this, object: CompletionHandler<Void, AsyncSocket> {
    ...
    suspend fun write(buf: ByteBuffer): Int {
        return suspendCancellableCoroutine { cont ->
            socket.write(buf, this, object : CompletionHandler<Int, AsyncSocket> {
    ...
    suspend fun read(buf: ByteBuffer): Int {
        return suspendCancellableCoroutine { cont ->
            socket.read(buf, this, object : CompletionHandler<Int,AsyncSocket> {
                override fun completed(result: Int, attachment: AsyncSocket) {
                    cont.resume(result)
                }
